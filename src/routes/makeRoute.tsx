...
Ns extends FlatNamespace,
  KPrefix extends KeyPrefix<FallbackNs<Ns>> = undefined,
> = {
  /** Page title (can be localized ref) */
  title?: MetaText<Ns, KPrefix>;

  /** Menu title (can be localized ref). Defaults to title if missing. */
  menuTitle?: MetaText<Ns, KPrefix>;

  /** Optional group label override (can be localized ref) */
  groupLabel?: MetaText<Ns, KPrefix>;

  /** Icon component for menus (client-rendered) */
  icon?: React.ComponentType<{ className?: string }>;

  /** Sort order for menu grouping (lower first) */
  order?: number;

  /** Hide this page from auto menus */
  hideInMenu?: boolean;

  /** Optional: include dynamic routes in menu */
  menuInclude?: boolean;

  /** Optional: explicit menu href for dynamic pages */
  menuHref?: string;

  /** Optional: fallback params/search to compute href for dynamic pages */
  menuParams?: Record<string, unknown>;
  menuSearch?: Record<string, unknown>;

  /** Optional: roles for menu visibility */
...
export interface DeleteInfo<Result extends z.ZodSchema> {
  result: Result;
}
...
> = CoreRouteElements<Params, Search> & {
  ...
  href: (p?: z.input<Params>, search?: z.input<Search>) => string;
  pattern: string;
  groups: string[];
  meta?: RouteMeta;
};
...
>(
  ...
  urlBuilder.href = (p?: z.input<Params>, s?: z.input<Search>) =>
    (urlBuilder as unknown as (p?: z.input<Params>, s?: z.input<Search>) => string)(p, s);

  urlBuilder.pattern = route;
  urlBuilder.groups = extractGroups(route);
  if (info.meta) urlBuilder.meta = info.meta;

  return urlBuilder;
}

function extractGroups(pattern: string): string[] {
